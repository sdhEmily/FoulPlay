...
// Inspect one slice. Adds slices to the regions array :)
static void scanSlice(int fd, uint64_t fileSize, uint64_t sliceOff,
    ...
    // for 32 bit slices the header size is different. It's fine to read mach_header_64 though below this
    // since it's just from file in and the struct is the same besides padding
    uint8_t hdr_size = sizeof(struct mach_header);
    ...
    // this wasn't needed
    // BOOL isBigEndian = (magic == MH_CIGAM_64 || magic == MH_CIGAM); 

    // if (isBigEndian) {
    //     FPLog(@"this slice is big endian");
    // }

    FPLog(@"cpu type: 0x%x, cpu subtype: %x", hdr.cputype, hdr.cpusubtype);

    if (magic == MH_MAGIC_64 || magic == MH_CIGAM_64) {
        FPLog(@"this slice is arm64");
        hdr_size = sizeof(struct mach_header_64);
        res->foundArm64 = YES;
    } else {
        FPLog(@"this slice is NOT arm64!");
    }
    ...
    uint64_t cmdOff = sliceOff + hdr_size;
    ...
    for (uint32_t i = 0; i < hdr.ncmds; i++) {
        ...
        // underflow.
        // FPLog(@"lc cmd type ox%x", lc.cmd);
        // if (lc.cmdsize < sizeof(lc) || lc.cmdsize > hdr.sizeofcmds - pos) {
        //     FPLog(@"broken cmdsize");
        //     break;
        // }

        if (lc.cmd == LC_ENCRYPTION_INFO_64 &&
            // 64 bit encryption info
            lc.cmdsize >= sizeof(struct encryption_info_command_64)) {
            struct encryption_info_command_64 enc;
            memcpy(&enc, cmds + pos, sizeof(enc));
            FPLog(@"Found a 64bit slice");
            if (enc.cryptid == 0) continue; // not encrypted
            res->regions[res->numRegions] = (EncryptionRegion){
...
static ScanResult* scanBinary(int fd, uint64_t fileSize, ScanResult* res) {
    uint32_t magic = 0;
    ...
    if (magic == FAT_CIGAM || magic == FAT_MAGIC) {
        ...
        res->readable = YES;
        // FPLog(@"fat header claims %u slices", nArch);
        // if (magic == FAT_CIGAM) {
        //     FPLog(@"this is a bigendian one also");
        // }
        ...
        res->readable = YES;
        FPLog(@"MH_MAGIC 64 found.");
        scanSlice(fd, fileSize, 0, fileSize, res);
    } else if (magic == MH_MAGIC || magic == MH_CIGAM) {
        res->readable = YES;
        FPLog(@"MH_MAGIC found.");
        // we support 32 bit arches now!
        scanSlice(fd, fileSize, 0, fileSize, res);
        // res.saw32bit = YES;
...
BOOL binaryNeedsDecryption(NSString *binaryPath, NSString *displayName,
    ...
    ScanResult res = {0};
    NSString *why = classify(*scanBinary(fd, (uint64_t)st.st_size, &res),
...
BOOL decryptBinaryAtPath(NSString *binaryPath,
    ...
    // since it could be perfectly fine just not have any regions lol
    if (scan.numRegions == 0) {
        close(fd); restoreCWD();
        *errorOut = @"No encrypted sections found.";
        return NO;
    }
    ...
    // we decrypt ALL the regions now!
    for (int i = 0; i < scan.numRegions; i++) {
        EncryptionRegion enc = scan.regions[i];
            ...
            if (ret != 0) {
                ...
                if (errorOut) {
                    if (savedErrno == EPERM && isPlatform) {
                        *errorOut = @"FairPlay keys missing. You need to have normally installed an app with this Apple ID at least once!";
                    } else {
